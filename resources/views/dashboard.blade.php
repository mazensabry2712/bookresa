...
@section('content')
    @php
        ...
        $statusLabels = [
            'pending' => __('dashboard.status_pending'),
            'confirmed' => __('dashboard.status_confirmed'),
            'rescheduled' => __('dashboard.status_rescheduled'),
            'completed' => __('dashboard.status_completed'),
            'cancelled' => __('dashboard.status_cancelled'),
            'no_show' => __('dashboard.status_no_show'),
        ];
        ...
        $paymentLabels = [
            'unpaid' => __('dashboard.payment_unpaid'),
            'partially_paid' => __('dashboard.payment_partial'),
            'paid' => __('dashboard.payment_paid'),
    ...
    <div class="space-y-6">
        <section class="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
            <div class="min-w-0">
                ...
                <p class="mt-2 max-w-2xl text-sm leading-6 text-slate-500 dark:text-slate-400">
                    {{ __('dashboard.workspace_summary') }}
                </p>
            </div>
            ...
            <div class="flex flex-wrap items-center gap-2">
                ...
                <a href="{{ route('public.booking.canonical.show', ['tenant' => $tenant->slug]) }}"
                   class="inline-flex min-h-11 items-center justify-center rounded-xl border border-slate-200 bg-white px-4 py-2.5 text-sm font-bold text-slate-700 transition hover:bg-slate-50 dark:border-slate-700 dark:bg-slate-900 dark:text-slate-200 dark:hover:bg-slate-800">
                    {{ __('dashboard.view_booking_page') }}
                </a>
            </div>
        </section>

        <section class="grid gap-4 sm:grid-cols-2 xl:grid-cols-4" aria-label="{{ __('dashboard.summary') }}">
            <article class="br-panel p-5">
                <div class="flex items-center justify-between gap-4">
                    ...
                    <span class="text-xs font-semibold text-slate-400">{{ __('dashboard.today') }}</span>
                </div>
                <p class="mt-4 text-sm font-semibold text-slate-500 dark:text-slate-400">{{ __('dashboard.today_bookings') }}</p>
            ...
            <article class="br-panel p-5">
                <div class="flex items-center justify-between gap-4">
                    ...
                    <span class="text-xs font-semibold text-slate-400">{{ __('dashboard.paid') }}</span>
                </div>
                <p class="mt-4 text-sm font-semibold text-slate-500 dark:text-slate-400">{{ __('dashboard.today_revenue') }}</p>
            ...
            <article class="br-panel p-5">
                <div class="flex items-center justify-between gap-4">
                    ...
                    <span class="text-xs font-semibold text-slate-400">{{ __('dashboard.upcoming') }}</span>
                </div>
                <p class="mt-4 text-sm font-semibold text-slate-500 dark:text-slate-400">{{ __('dashboard.upcoming_bookings') }}</p>
            ...
            <article class="br-panel p-5">
                ...
                <p class="mt-4 text-sm font-semibold text-slate-500 dark:text-slate-400">{{ __('dashboard.new_customers') }}</p>
        ...
        <section class="br-panel overflow-hidden" aria-labelledby="attention-title">
            <div class="flex items-center justify-between gap-4 border-b border-slate-200 px-5 py-4 dark:border-slate-800">
                <div>
                    <h3 id="attention-title" class="font-semibold text-slate-950 dark:text-white">{{ __('dashboard.attention') }}</h3>
                    <p class="mt-1 text-sm text-slate-500 dark:text-slate-400">{{ __('dashboard.attention_description') }}</p>
                </div>
            </div>
                ...
                @if ($attention['pendingBookings'] > 0)
                    <a href="{{ route('booking.management.index') }}" class="flex items-center justify-between gap-4 px-5 py-4 transition hover:bg-slate-50 dark:hover:bg-slate-950/40">
                        <div class="flex min-w-0 items-start gap-3">
                            ...
                            <div class="min-w-0">
                                <p class="font-semibold text-slate-900 dark:text-white">{{ trans_choice('dashboard.pending_bookings_count', $attention['pendingBookings']) }}</p>
                                <p class="mt-0.5 text-sm text-slate-500">{{ __('dashboard.pending_bookings_help') }}</p>
                            </div>
                        </div>
                        <span class="shrink-0 text-sm font-bold text-brand-indigo">{{ __('dashboard.review') }}</span>
                    </a>
                @endif
                ...
                @if ($attention['unpaidBookings'] > 0)
                    <a href="{{ route('booking.management.index') }}" class="flex items-center justify-between gap-4 px-5 py-4 transition hover:bg-slate-50 dark:hover:bg-slate-950/40">
                        <div class="flex min-w-0 items-start gap-3">
                            ...
                            <div class="min-w-0">
                                <p class="font-semibold text-slate-900 dark:text-white">{{ trans_choice('dashboard.unpaid_bookings_count', $attention['unpaidBookings']) }}</p>
                                <p class="mt-0.5 text-sm text-slate-500">{{ __('dashboard.unpaid_bookings_help') }}</p>
                            </div>
                        </div>
                        <span class="shrink-0 text-sm font-bold text-brand-indigo">{{ __('dashboard.review') }}</span>
                    </a>
                        ...
                        <div class="flex min-w-0 items-start gap-3">
                            ...
                            <div class="min-w-0">
                                <p class="font-semibold text-slate-900 dark:text-white">
                                    @if ($subscription)
                                        {{ __('dashboard.subscription_expires_soon') }}
                                    @else
                                ...
                                <p class="mt-0.5 text-sm text-slate-500">
                                    @if ($subscription)
                                        {{ trans_choice('dashboard.subscription_days_left', $subscriptionDaysRemaining ?? 0, ['count' => $subscriptionDaysRemaining ?? 0]) }}
                                    @else
                                        {{ __('dashboard.subscription_action_help') }}
                                    @endif
                                </p>
                            </div>
                        </div>
                        <span class="shrink-0 text-sm font-bold text-brand-indigo">{{ __('dashboard.manage') }}</span>
                    </a>
                @endif
                            ...
                            <div class="min-w-0">
                                <p class="font-semibold text-slate-900 dark:text-white">
                                    @if ($attention['usageOverLimit'])
                                        {{ __('dashboard.usage_over_limit') }}
                                    @else
                                        {{ __('dashboard.usage_high') }}
                                    @endif
                                </p>
                                <p class="mt-0.5 text-sm text-slate-500">
                                    {{ __('dashboard.usage_help', ['percent' => $attention['usagePercent'] ?? 0]) }}
                                </p>
                            </div>
                        </div>
                        <span class="shrink-0 text-sm font-bold text-brand-indigo">{{ __('dashboard.manage') }}</span>
                ...
                @if (
                    ...
                    <div class="px-5 py-6">
                        <div class="flex items-start gap-3 rounded-xl border border-emerald-200 bg-emerald-50 px-4 py-3 dark:border-emerald-900/70 dark:bg-emerald-950/25">
                            ...
                            <div>
                                <p class="font-semibold text-emerald-800 dark:text-emerald-200">{{ __('dashboard.no_attention_items') }}</p>
                                <p class="mt-0.5 text-sm text-emerald-700 dark:text-emerald-300">{{ __('dashboard.no_attention_help') }}</p>
                            </div>
                        </div>
                    </div>
                @endif
        ...
        <div class="grid gap-6 xl:grid-cols-[1.6fr_0.9fr]">
            <section class="br-panel overflow-hidden" aria-labelledby="upcoming-title">
                <div class="flex items-center justify-between gap-4 border-b border-slate-200 px-5 py-4 dark:border-slate-800">
                    <div>
                        <h3 id="upcoming-title" class="font-semibold text-slate-950 dark:text-white">{{ __('dashboard.upcoming_bookings') }}</h3>
                        <p class="mt-1 text-sm text-slate-500 dark:text-slate-400">{{ __('dashboard.upcoming_description') }}</p>
                    </div>
                    @can('bookings.view')
                        <a href="{{ route('booking.management.index') }}" class="shrink-0 text-sm font-bold text-brand-indigo hover:underline">{{ __('dashboard.view_all') }}</a>
                    @endcan
                </div>
                            ...
                            <div class="grid gap-4 lg:grid-cols-[auto_1fr_auto_auto] lg:items-center">
                                ...
                                <div class="min-w-0">
                                    ...
                                    <p class="mt-1 truncate text-sm text-slate-500">
                                        ...
                                        {{ $booking->staff?->display_name ?? __('dashboard.auto_assigned') }}
                                    </p>
                    ...
                    @empty
                        <div class="px-5 py-12">
                            <div class="mx-auto max-w-md text-center">
                                ...
                                <p class="mt-4 font-semibold text-slate-900 dark:text-white">{{ __('dashboard.no_upcoming_bookings') }}</p>
                                <p class="mt-1 text-sm leading-6 text-slate-500">{{ __('dashboard.no_upcoming_help') }}</p>
            ...
            <aside class="space-y-6">
                <section class="br-panel p-5" aria-labelledby="subscription-title">
                    <div class="flex items-start justify-between gap-4">
                        <div>
                            <p class="text-xs font-bold uppercase tracking-[0.14em] text-slate-400">{{ __('dashboard.subscription') }}</p>
                            <h3 id="subscription-title" class="mt-2 text-xl font-bold text-slate-950 dark:text-white">{{ $planName }}</h3>
                        </div>
                        <a href="{{ route('billing.subscription') }}" class="text-sm font-bold text-brand-indigo hover:underline">{{ __('dashboard.manage') }}</a>
                    </div>

                    @if ($subscription)
                        <div class="mt-4 grid grid-cols-2 gap-3">
                            <div class="rounded-xl br-surface-soft p-3">
                                <p class="text-xs font-semibold text-slate-500">{{ __('dashboard.subscription_status') }}</p>
                                <p class="mt-1 font-bold text-slate-900 dark:text-white">{{ str($subscription->status->value)->headline() }}</p>
                            </div>
                            <div class="rounded-xl br-surface-soft p-3">
                                <p class="text-xs font-semibold text-slate-500">{{ __('dashboard.renews') }}</p>
                ...
                @if ($usageSummary)
                    <section class="br-panel p-5" aria-labelledby="usage-title">
                        <div class="flex items-center justify-between gap-4">
                            <div>
                                <p class="text-xs font-bold uppercase tracking-[0.14em] text-slate-400">{{ __('dashboard.customer_usage') }}</p>
                        ...
                        <p class="mt-3 text-sm text-slate-500">
                            {{ __('dashboard.customers_count', ['count' => number_format($usageSummary->uniqueCustomerCount)]) }}
                        </p>
                        ...
                        @if ($usageSummary->additionalCustomerCount > 0)
                            <p class="mt-3 rounded-xl border border-amber-200 bg-amber-50 px-3 py-2.5 text-sm font-medium text-amber-800 dark:border-amber-900/70 dark:bg-amber-950/25 dark:text-amber-200">
                                {{ __('dashboard.additional_customers', ['count' => number_format($usageSummary->additionalCustomerCount)]) }}
                            </p>
                        @endif
                    </section>
                @endif
                ...
                <section class="br-panel p-5" aria-labelledby="workspace-title">
                    <div class="flex items-center justify-between gap-4">
                        <div>
                            <p class="text-xs font-bold uppercase tracking-[0.14em] text-slate-400">{{ __('dashboard.workspace') }}</p>
                        ...
                        <span class="rounded-full border border-slate-200 px-2.5 py-1 text-xs font-bold text-slate-600 dark:border-slate-700 dark:text-slate-300">
                            {{ trans_choice('dashboard.active_staff_count', $metrics['activeStaff'], ['count' => $metrics['activeStaff']]) }}
                        </span>
                    </div>
