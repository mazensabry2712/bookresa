...
@section('title', __('app.business_ui.business_profile').' — '.config('app.name', 'BookResa'))
@section('heading', __('app.business_ui.business_profile'))
    ...
    <div class="max-w-6xl space-y-6">
            ...
            <div class="relative h-32 bg-brand-navy sm:h-40">
                @if ($profile->cover_path)
                    <img src="{{ Storage::disk('public')->url($profile->cover_path) }}"
                         alt="{{ __('app.business_ui.cover_image') }}"
                ...
                <div class="-mt-10 flex flex-col gap-4 sm:-mt-12 sm:flex-row sm:items-end sm:justify-between">
                    <div class="flex min-w-0 items-end gap-4">
                        ...
                        <div class="min-w-0 pb-1">
                            <p class="text-xs font-bold uppercase tracking-[0.14em] text-slate-400">{{ __('app.business_ui.business_profile') }}</p>
                    ...
                    <a href="{{ $bookingUrl }}"
                       ...
                       class="inline-flex min-h-11 items-center justify-center rounded-xl bg-brand-indigo px-4 py-2.5 text-sm font-bold text-white transition hover:bg-indigo-600">
                        {{ __('app.business_ui.open_booking_page') }}
                    </a>
                </div>
            </div>
        </section>
        ...
        <form method="POST" action="{{ route('business.profile.update') }}" enctype="multipart/form-data" class="space-y-6">
            ...
            <section class="br-panel p-5 sm:p-6">
                <div>
                    <h3 class="font-semibold text-slate-950 dark:text-white">{{ __('app.business_ui.basic_information') }}</h3>
                    <p class="mt-1 text-sm leading-6 text-slate-500">{{ __('app.business_ui.basic_information_help') }}</p>
                </div>

                <div class="mt-5 grid gap-4 sm:grid-cols-2">
                    <label class="space-y-1.5 text-sm">
                        <span class="font-semibold">{{ __('app.business_ui.name_english') }}</span>
                    ...
                    <label class="space-y-1.5 text-sm">
                        <span class="font-semibold">{{ __('app.business_ui.name_arabic') }}</span>
                    ...
                    <label class="space-y-1.5 text-sm sm:col-span-2">
                        <span class="font-semibold">{{ __('app.business_ui.description_english') }}</span>
                    ...
                    <label class="space-y-1.5 text-sm sm:col-span-2">
                        <span class="font-semibold">{{ __('app.business_ui.description_arabic') }}</span>
            ...
            <section class="br-panel p-5 sm:p-6">
                <div>
                    <h3 class="font-semibold text-slate-950 dark:text-white">{{ __('app.business_ui.branding') }}</h3>
                    <p class="mt-1 text-sm leading-6 text-slate-500">{{ __('app.business_ui.branding_help') }}</p>
                </div>

                <div class="mt-5 grid gap-6 lg:grid-cols-2">
                    <div>
                        <p class="text-sm font-semibold">{{ __('app.business_ui.logo') }}</p>
                        <div class="mt-3 flex items-center gap-4">
                            <div class="flex h-20 w-20 shrink-0 items-center justify-center overflow-hidden rounded-2xl bg-brand-soft text-xl font-extrabold text-brand-navy dark:bg-slate-800 dark:text-indigo-300">
                                @if ($profile->logo_path)
                                    <img src="{{ Storage::disk('public')->url($profile->logo_path) }}" alt="{{ __('app.business_ui.logo') }}" class="h-full w-full object-cover">
                                @else
                                    {{ str($localized($profile->name))->substr(0, 1)->upper() }}
                                @endif
                            </div>
                            <div class="min-w-0 flex-1">
                                ...
                                <p class="mt-1 text-xs leading-5 text-slate-500">{{ __('app.business_ui.logo_help') }}</p>
                            </div>
                        </div>
                        @if ($profile->logo_path)
                            <label class="mt-4 flex items-center gap-2 text-sm">
                                <input type="checkbox" name="remove_logo" value="1" class="rounded border-slate-300">
                                {{ __('app.business_ui.remove_logo') }}
                            </label>
                        @endif
                    </div>

                    <div>
                        <p class="text-sm font-semibold">{{ __('app.business_ui.cover_image') }}</p>
                        <div class="mt-3 overflow-hidden rounded-2xl bg-brand-soft dark:bg-slate-800">
                            @if ($profile->cover_path)
                                <img src="{{ Storage::disk('public')->url($profile->cover_path) }}" alt="{{ __('app.business_ui.cover_image') }}" class="h-28 w-full object-cover">
                            @else
                                <div class="flex h-28 items-center justify-center text-sm font-medium text-slate-500">{{ __('app.business_ui.no_cover') }}</div>
                            @endif
                        </div>
                        ...
                        <p class="mt-1 text-xs leading-5 text-slate-500">{{ __('app.business_ui.cover_help') }}</p>
                        @if ($profile->cover_path)
                            <label class="mt-3 flex items-center gap-2 text-sm">
                                <input type="checkbox" name="remove_cover" value="1" class="rounded border-slate-300">
                                {{ __('app.business_ui.remove_cover') }}
                            </label>
                        @endif
                    </div>
                </div>
            ...
            <section class="br-panel p-5 sm:p-6">
                <div>
                    <h3 class="font-semibold text-slate-950 dark:text-white">{{ __('app.business_ui.contact_location') }}</h3>
                    <p class="mt-1 text-sm leading-6 text-slate-500">{{ __('app.business_ui.contact_location_help') }}</p>
                </div>

                <div class="mt-5 grid gap-4 sm:grid-cols-2">
                    <label class="space-y-1.5 text-sm">
                        <span class="font-semibold">{{ __('app.business_ui.phone') }}</span>
                        <input name="phone" value="{{ old('phone', $profile->phone) }}" maxlength="40" inputmode="tel"
                               class="w-full rounded-xl border border-slate-300 bg-white px-3.5 py-3 dark:border-slate-700 dark:bg-slate-950">
                    </label>

                    <label class="space-y-1.5 text-sm">
                        <span class="font-semibold">{{ __('app.business_ui.email') }}</span>
                        <input type="email" name="email" value="{{ old('email', $profile->email) }}" maxlength="255"
                               class="w-full rounded-xl border border-slate-300 bg-white px-3.5 py-3 dark:border-slate-700 dark:bg-slate-950">
                    </label>

                    <label class="space-y-1.5 text-sm">
                        <span class="font-semibold">{{ __('app.business_ui.location') }}</span>
                        <input name="location" value="{{ old('location', $profile->location) }}" maxlength="255"
                               class="w-full rounded-xl border border-slate-300 bg-white px-3.5 py-3 dark:border-slate-700 dark:bg-slate-950">
                    </label>

                    <label class="space-y-1.5 text-sm">
                        <span class="font-semibold">{{ __('app.business_ui.timezone') }}</span>
                    ...
                    <label class="space-y-1.5 text-sm sm:col-span-2">
                        <span class="font-semibold">{{ __('app.business_ui.address') }}</span>
            ...
            <section class="br-panel p-5 sm:p-6">
                ...
                <div class="mt-5 space-y-6">
                    <div>
                        <p class="text-sm font-semibold">{{ __('app.business_ui.payment_mode') }}</p>
                        @if ($paymentsEnabled)
                            <div class="mt-3 grid gap-3 sm:grid-cols-3">
                                @foreach ([
                                    'full' => ['title' => __('app.business_ui.full_payment'), 'help' => __('app.business_ui.full_payment_help')],
                                    'deposit' => ['title' => __('app.business_ui.deposit'), 'help' => __('app.business_ui.deposit_help')],
                                    'pay_later' => ['title' => __('app.business_ui.pay_later'), 'help' => __('app.business_ui.pay_later_help')],
                                ] as $value => $option)
                            ...
                            <div class="mt-4 max-w-xs">
                                <label class="space-y-1.5 text-sm">
                                    <span class="font-semibold">{{ __('app.business_ui.deposit_percent') }}</span>
                        ...
                        @else
                            ...
                            <p class="mt-3 rounded-xl border border-slate-200 bg-slate-50 px-4 py-3 text-sm text-slate-600 dark:border-slate-800 dark:bg-slate-950 dark:text-slate-300">
                                {{ __('app.business_ui.payments_module_disabled') }}
                            </p>
                        @endif
                    </div>
                    ...
                    <div class="grid gap-4 sm:grid-cols-2">
                        <label class="space-y-1.5 text-sm">
                            <span class="font-semibold">{{ __('app.business_ui.minimum_notice') }}</span>
                            <input type="number" name="minimum_notice_minutes" min="0" max="43200"
                                   ...
                                   class="w-full rounded-xl border border-slate-300 bg-white px-3.5 py-3 dark:border-slate-700 dark:bg-slate-950">
                            <span class="block text-xs leading-5 text-slate-500">{{ __('app.business_ui.minimum_notice_help') }}</span>
                        </label>

                        <label class="space-y-1.5 text-sm">
                            <span class="font-semibold">{{ __('app.business_ui.maximum_advance') }}</span>
                            <input type="number" name="maximum_advance_days" min="1" max="730"
                                   ...
                                   placeholder="{{ __('app.business_ui.no_limit') }}"
                                   class="w-full rounded-xl border border-slate-300 bg-white px-3.5 py-3 dark:border-slate-700 dark:bg-slate-950">
                            <span class="block text-xs leading-5 text-slate-500">{{ __('app.business_ui.maximum_advance_help') }}</span>
                        </label>
                    </div>
                    ...
                    <div class="grid gap-4 sm:grid-cols-2">
                        <label class="flex items-start gap-3 rounded-xl br-surface-soft p-4 text-sm">
                            ...
                            <span>
                                <span class="block font-semibold">{{ __('app.business_ui.require_email') }}</span>
                                <span class="mt-1 block text-xs leading-5 text-slate-500">{{ __('app.business_ui.require_email_help') }}</span>
                            </span>
                        </label>

                        <label class="space-y-1.5 text-sm">
                            <span class="font-semibold">{{ __('app.business_ui.customer_limit_policy') }}</span>
                            <select name="customer_limit_policy" class="w-full rounded-xl border border-slate-300 bg-white px-3.5 py-3 dark:border-slate-700 dark:bg-slate-950">
                                <option value="allow_overage" @selected($customerLimitPolicy === 'allow_overage')>{{ __('app.business_ui.allow_overage') }}</option>
                                <option value="block_new_customers" @selected($customerLimitPolicy === 'block_new_customers')>{{ __('app.business_ui.block_new_customers') }}</option>
                            </select>
                        </label>
                    </div>
                </div>
            ...
            <section class="br-panel p-5 sm:p-6">
                <div>
                    <h3 class="font-semibold text-slate-950 dark:text-white">{{ __('app.business_ui.language_appearance') }}</h3>
                    <p class="mt-1 text-sm leading-6 text-slate-500">{{ __('app.business_ui.language_appearance_help') }}</p>
                </div>

                <div class="mt-5 grid gap-4 lg:grid-cols-2">
                    <div class="rounded-xl br-surface-soft p-4">
                        <p class="text-sm font-semibold">{{ __('app.business_ui.interface_language') }}</p>
                        <p class="mt-1 text-xs leading-5 text-slate-500">{{ __('app.business_ui.interface_language_help') }}</p>
                    ...
                    <div class="rounded-xl br-surface-soft p-4">
                        <p class="text-sm font-semibold">{{ __('app.business_ui.theme') }}</p>
                        <p class="mt-1 text-xs leading-5 text-slate-500">{{ __('app.business_ui.theme_help') }}</p>
                        <div class="mt-4 w-fit">
                            <x-theme-toggle />
                        </div>
                    </div>
            ...
            <section class="br-panel p-5 sm:p-6">
                <div>
                    <h3 class="font-semibold text-slate-950 dark:text-white">{{ __('app.business_ui.public_presence') }}</h3>
                    <p class="mt-1 text-sm leading-6 text-slate-500">{{ __('app.business_ui.public_presence_help') }}</p>
                </div>

                <div class="mt-5 rounded-xl border border-indigo-100 bg-indigo-50/60 p-4 dark:border-indigo-900/50 dark:bg-indigo-950/20">
                    <p class="text-xs font-bold uppercase tracking-[0.14em] text-indigo-600 dark:text-indigo-300">{{ __('app.business_ui.booking_url') }}</p>
                ...
                <div class="mt-5 grid gap-4 sm:grid-cols-3">
                    @foreach ([
                        'website' => __('app.business_ui.website'),
                        'facebook' => __('app.business_ui.facebook'),
                        'instagram' => __('app.business_ui.instagram'),
                    ] as $key => $label)
                        <label class="space-y-1.5 text-sm">
            ...
            <section class="br-panel p-5 sm:p-6">
                <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                    <div>
                        <h3 class="font-semibold text-slate-950 dark:text-white">{{ __('app.business_ui.team_roles') }}</h3>
            ...
            <div class="flex flex-col-reverse gap-3 sm:flex-row sm:items-center sm:justify-between">
                <p class="text-xs leading-5 text-slate-500">{{ __('app.business_ui.save_help') }}</p>
                @can('business.update')
                    <button type="submit"
                        ...
                        {{ __('app.business_ui.save_changes') }}
                    </button>
                @endcan
            </div>
        </form>
        ...
        document.addEventListener('DOMContentLoaded', () => {
            ...
            button.addEventListener('click', async () => {
                try {
                    ...
                    feedback.textContent = @json(__('app.business_ui.url_copied'));
                } catch {
                    input.select();
                    feedback.textContent = @json(__('app.business_ui.copy_fallback'));
                }
            });
        });
    </script>
