...
@section('title', __('app.service_ui.services').' — '.config('app.name', 'BookResa'))
...
@section('content')
    ...
    <div class="space-y-6">
        <section class="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
            <div>
                <p class="text-sm font-semibold text-brand-indigo">{{ __('app.service_ui.workspace_operations') }}</p>
                <h2 class="mt-1 text-2xl font-bold tracking-tight text-slate-950 dark:text-white">{{ __('app.service_ui.services') }}</h2>
                <p class="mt-2 max-w-2xl text-sm leading-6 text-slate-500">{{ __('app.service_ui.page_help') }}</p>
            </div>
                ...
                <div class="flex items-start justify-between gap-4">
                    <div>
                        <p class="text-xs font-bold uppercase tracking-[0.14em] text-slate-400">
                            {{ $isEditing ? __('app.service_ui.editing') : __('app.service_ui.new_service') }}
                        </p>
                        <h3 class="mt-2 text-xl font-bold text-slate-950 dark:text-white">
                            {{ $isEditing ? __('app.service_ui.edit_service') : __('app.service_ui.create_service') }}
                        </h3>
                        <p class="mt-1 text-sm leading-6 text-slate-500">{{ __('app.service_ui.form_help') }}</p>
                    </div>
                ...
                <form method="POST"
                    ...
                    <div class="grid gap-4 sm:grid-cols-2">
                        <label class="space-y-1.5 text-sm">
                            <span class="font-semibold">{{ __('app.service_ui.name_english') }}</span>
                            <input name="name_en"
                        ...
                        <label class="space-y-1.5 text-sm">
                            <span class="font-semibold">{{ __('app.service_ui.name_arabic') }}</span>
                            <input name="name_ar"
                        ...
                        <label class="space-y-1.5 text-sm sm:col-span-2">
                            <span class="font-semibold">{{ __('app.service_ui.description_english') }}</span>
                        ...
                        <label class="space-y-1.5 text-sm sm:col-span-2">
                            <span class="font-semibold">{{ __('app.service_ui.description_arabic') }}</span>
                    ...
                    <div class="grid gap-4 sm:grid-cols-2">
                        <label class="space-y-1.5 text-sm">
                            <span class="font-semibold">{{ __('app.service_ui.price') }}</span>
                            <input name="price"
                        ...
                        <label class="space-y-1.5 text-sm">
                            <span class="font-semibold">{{ __('app.service_ui.currency') }}</span>
                            <input name="currency"
                        ...
                        <label class="space-y-1.5 text-sm">
                            <span class="font-semibold">{{ __('app.service_ui.duration') }}</span>
                        ...
                        <label class="space-y-1.5 text-sm">
                            <span class="font-semibold">{{ __('app.service_ui.buffer') }}</span>
                    ...
                    <label class="flex items-start gap-3 rounded-xl br-surface-soft p-4 text-sm">
                        ...
                        <span>
                            <span class="block font-semibold">{{ __('app.service_ui.active_service') }}</span>
                            <span class="mt-1 block text-xs leading-5 text-slate-500">{{ __('app.service_ui.active_service_help') }}</span>
                        </span>
                    </label>
                    ...
                    <div class="flex flex-col gap-2 sm:flex-row">
                        <button type="submit"
                            ...
                            {{ $isEditing ? __('app.service_ui.update_service') : __('app.service_ui.create_service') }}
                        </button>
                                ...
                                <div class="min-w-0">
                                    <div class="flex flex-wrap items-center gap-2">
                                        ...
                                        <span class="inline-flex items-center rounded-full border px-2.5 py-1 text-xs font-bold {{ $service->is_active ? 'border-emerald-200 bg-emerald-50 text-emerald-700 dark:border-emerald-900 dark:bg-emerald-950/30 dark:text-emerald-300' : 'border-slate-200 bg-slate-100 text-slate-600 dark:border-slate-700 dark:bg-slate-800 dark:text-slate-300' }}">
                                            {{ $service->is_active ? __('app.service_ui.active') : __('app.service_ui.inactive') }}
                                        </span>
                                    </div>
                    ...
                    @empty
                        <div class="px-5 py-12">
                            <div class="mx-auto max-w-md text-center">
                                ...
                                <h4 class="mt-4 font-semibold text-slate-950 dark:text-white">{{ __('app.service_ui.no_services') }}</h4>
                                <p class="mt-1 text-sm leading-6 text-slate-500">{{ __('app.service_ui.no_services_help') }}</p>
                                <a href="#service-form"
                                   class="mt-5 inline-flex min-h-10 items-center justify-center rounded-xl bg-brand-indigo px-4 py-2.5 text-sm font-bold text-white hover:bg-indigo-600">
                                    {{ __('app.service_ui.create_service') }}
                                </a>
                            </div>
                        </div>
                    @endforelse
